...
def predict_risk_with_range(
    ...
    # Missing = NaN/None in optional fields
    missing_or_unknown = [c for c in OPTIONAL_FIELDS if pd.isna(X_base.loc[0, c])]

    # Also treat flagged unknown fields as missing for range calc (even if we passed a fallback)
    ...
    for col in missing_or_unknown:
        ...
        # Safety fallback: if we have no plausible candidates, fall back to 0 or current value
