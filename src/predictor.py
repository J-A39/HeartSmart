...
# ----------------------------
# Risk bands (separate from model threshold)
# ----------------------------
def risk_band(risk: float) -> str:
    # Simple, human-friendly bands for a 10-year risk
    if risk < 0.05:
        return "Low"
    if risk < 0.10:
        return "Borderline"
...
def _counterfactual_nonsmoker_risk(
    ...
    """
    Compute risk if the user were a non-smoker (currentSmoker=0, cigsPerDay=0).
    Returns None if required fields not present.
    """
...
def _apply_guardrails(
    *,
    ...
    """
    Minimal guardrails so your saved/logged predictions are stable and sensible.
    We only apply risk *floors* (never force risk down).
    """
    warnings: List[str] = []
    ...
    # --- Extreme-value floors (minimal, conservative) ---
    # These are intentionally simple so they remain stable over time.
    ...
    # --- Smoking monotonic sanity check ---
    # If the model+imputation produces a lower risk for smoker vs non-smoker counterfactual,
    # bump up to the non-smoker counterfactual (stable, conservative).
    try:
        ...
        # Never break prediction due to guardrails
        pass

    adjusted = _clamp01(adjusted)

    # Ensure the displayed range still makes sense w.r.t. the adjusted base
...
def predict(
    *,
    ...
    # Versioning: safe defaults if you didn't bake it into the artifact yet
    model_version = str(artifact.get("model_version", "v1"))
    rules_version = str(artifact.get("rules_version", "v1"))

    # Force feature order / presence
    ...
    # Apply minimal guardrails so persisted results stay sensible
